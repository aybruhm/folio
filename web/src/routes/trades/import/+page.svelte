<script lang="ts">
    ...
    const TRADE_FIELDS = [
        ...
        },
        {
            ...
            required: false,
            hint: "yfinance, tiingo, ngnmarket, ngxpulse, tradingview",
        },
    ...
    $: requiredMapped = TRADE_FIELDS.filter((f) => f.required).every(
        (f) => mapping[f.key],
    );
    ...
    async function validate() {
        ...
        try {
            ...
            validation = await controller.validateCsv(
                file,
                mapping,
                dateFormat,
            );
    ...
    <div class="mx-auto max-w-3xl space-y-6">
        ...
        <div class="flex items-center gap-0">
            {#each STEPS as label, i}
              ...
              {step === n
                    ...
                    {#if i < STEPS.length - 1}
                        <div
                            class="flex-1 h-px mx-2 mt-[-1rem] {step > n
                                ? 'bg-primary/40'
                                : 'bg-border'}"
                        ></div>
                    {/if}
        ...
        {#if step === 1}
            <Card
                ...
                subtitle="Select a CSV file with your trade history"
            >
                <!-- Drop zone -->
                <label
                                ...
                                <tbody>
                                    {#each sampleRows as row}
                                        <tr class="border-t border-border">
                                            {#each headers as h}
                                                <td
                                                    class="px-3 py-2 text-foreground whitespace-nowrap"
                                                    >{row[h] ?? ""}</td
                                                >
                                            {/each}
                                        </tr>
        ...
        {#if step === 2}
            <Card
                title="Map Columns"
                subtitle="Match your CSV headers to the expected trade fields"
            >
                <div class="space-y-3">
            ...
            <Card
                title="Date Format"
                subtitle="How are dates written in your CSV?"
            >
                <div class="space-y-3">
                    <Select
                        label="Format preset"
        ...
        {#if step === 3 && validation}
            <Card title="Review Import">
                ...
                <div class="flex flex-wrap gap-3 mb-4">
                    <div
                        class="flex items-center gap-2 rounded-lg border border-border px-4 py-2"
                    >
                        <span class="text-2xl font-bold text-positive"
                            >{validation.valid_count}</span
                        >
                    ...
                    {#if validation.error_count > 0}
                        <div
                            class="flex items-center gap-2 rounded-lg border border-border px-4 py-2"
                        >
                            <span class="text-2xl font-bold text-negative"
                                >{validation.error_count}</span
                            >
                            <span class="text-sm text-muted-foreground"
                                >errors</span
                            >
                ...
                {#if validation.error_count > 0}
                    <div class="mb-4">
                        ...
                        >
                            <svg
                                class="h-4 w-4 transition-transform {errorsExpanded
                                    ? 'rotate-90'
                                    : ''}"
                                fill="none"
                                stroke="currentColor"
                            ...
                            {validation.error_count} error{validation.error_count !==
                            1
                                ? "s"
                                : ""}
                        </button>
                        {#if errorsExpanded}
                            <div
                                class="rounded-md border border-border divide-y divide-border max-h-48 overflow-y-auto"
                            >
                    ...
                    <div class="space-y-2">
                        ...
                        >
                            <table class="w-full text-xs">
                                <thead class="bg-muted">
                                    <tr>
                                        {#each ["ticker", "trade_type", "trade_date", "quantity", "price", "currency", "market_data_provider"] as col}
                                            <th
                                                class="px-3 py-2 text-left font-medium text-muted-foreground capitalize whitespace-nowrap"
                                            >
                                                {col.replace("_", " ")}
                                            </th>
                                        {/each}
            ...
            <div class="flex justify-between gap-3">
                <Button variant="outline" on:click={() => (step = 2)}
                    >Back</Button
                >
                <Button
                    variant="default"
                ...
                >
                    {importing
                        ? "Importing…"
                        : `Import ${validation.valid_count} trade${validation.valid_count !== 1 ? "s" : ""}`}
                </Button>
            </div>
        {/if}
    </div>
