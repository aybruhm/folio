<svelte:head>
    <title>{portfolio ? `${portfolio.name} — Folio` : 'Portfolio — Folio'}</title>
</svelte:head>
...
<script lang="ts">
  ...
  async function loadData() {
    try {
      loading = true
      const [portfolioData, holdingsData, analyticsData] = await Promise.all([
        portfolioController.getPortfolio(portfolioId),
        portfolioController.getHoldings({ portfolio_id: portfolioId }).catch(() => ({ data: [], currency: '', total_value: 0 })),
        portfolioController.getPortfolioAnalytics({ portfolio_id: portfolioId, timeframe: '1y' }).catch(() => null)
      ])
  ...
  <div class="mx-auto max-w-6xl space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div class="flex items-center gap-4">
        ...
        {#if portfolio}
          <div>
            <h1 class="text-2xl md:text-3xl font-bold text-foreground">{portfolio.name}</h1>
            <p class="text-xs md:text-sm text-muted-foreground">{portfolio.base_currency}</p>
          </div>
        {/if}
      </div>
      {#if portfolio}
        <Button variant="outline" size="sm" href="/trades?portfolio_id={portfolioId}">
          Add Trade
        </Button>
      {/if}
    </div>
    ...
    {:else if !portfolio}
      <Card title="Portfolio not found" subtitle="This portfolio does not exist or could not be loaded.">
        <div class="text-center py-8">
          <Button variant="default" href="/portfolios">Back to Portfolios</Button>
        </div>
      </Card>
    {:else}
      <!-- Stats -->
      {#if analytics}
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <Card title="Current Value">
            <p class="text-xl font-bold text-foreground">{formatCurrency(analytics.current_value)}</p>
          </Card>
      ...
      <!-- Holdings -->
      <Card title="Holdings" subtitle="Current positions in this portfolio">
